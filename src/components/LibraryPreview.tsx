...
const LibraryPreview = ({
  ...
  return <div className={`${isFullScreenMode ? 'library-fullscreen' : 'terminal-container w-full'} ${isDemo ? 'relative' : ''}`}>
      ...
      {/* Only show controls when not in full screen zen mode */}
      {!isFullScreenMode && <div className="flex justify-between items-center mb-4">
          <h3 className="terminal-header text-2xl">Your Unplayed Library</h3>
          
          <div className="flex space-x-2">
            <button onClick={() => setViewMode('grid')} className={`px-3 py-1 text-sm rounded-md ${viewMode === 'grid' ? 'bg-unplayed-mint text-black' : 'bg-gray-800 text-gray-300'}`}>
              Grid
            </button>
            <button onClick={() => setViewMode('zen')} className={`px-3 py-1 text-sm rounded-md ${viewMode === 'zen' ? 'bg-unplayed-mint text-black' : 'bg-gray-800 text-gray-300'}`}>
              Zen
            </button>
      ...
      {/* Grid view mode */}
      {viewMode === 'grid' ? <div className={`grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 mb-4 ${isFullScreenMode ? 'p-8' : ''}`}>
          {sampleGames.map(game => <div key={game.id} className={`relative overflow-hidden rounded-md transition-transform duration-300 hover:scale-105 ${isFullScreenMode ? 'library-game-fullscreen' : ''}`} onMouseEnter={() => setHoveredGame(game.id)} onMouseLeave={() => setHoveredGame(null)}>
    ...
    <div className={`${isFullScreenMode ? 'h-screen' : 'h-64'} overflow-hidden relative`}>
          ...
          {sampleGames.map((game, index) => <div key={game.id} className="absolute opacity-0 transition-all duration-[4s] animate-fade-in" style={{
        top: `${Math.random() * 80}%`,
        left: `${Math.random() * 80}%`,
      ...
      {!isFullScreenMode && <div className="text-center mt-6">
          <p className="text-gray-400">
            Showing 10 of 137 unplayed games
          </p>
          <div className="flex justify-center gap-2 mt-3">
            {isDemo ? <button onClick={() => signInWithSteam()} className="btn-secondary">
                Connect Steam to View Your Library
              </button> : <button className="btn-secondary">
                View Full Library
              </button>}
            <button onClick={() => enterZenMode('library')} className="btn-primary flex items-center" title="Enter full-screen mode">
