...
const LibraryPreview = ({ 
  ...
  return (
    <div className={`${isFullScreenMode ? 'library-fullscreen' : 'terminal-container w-full'} ${isDemo ? 'relative' : ''}`}>
      ...
      {/* Grid view mode */}
      {viewMode === 'grid' ? (
        <div className={`grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 mb-4 ${isFullScreenMode ? 'p-8' : ''}`}>
          {sampleGames.map((game) => (
            ...
            >
              ...
              <div className={`absolute inset-0 bg-gradient-to-t from-black/80 to-transparent p-2 flex flex-col justify-end transition-opacity duration-300 ${
                hoveredGame === game.id || isFullScreenMode ? 'opacity-100' : 'opacity-0'
              }`}>
