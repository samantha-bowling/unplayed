...
const GenreHoarding = ({
  ...
  return <div className={`terminal-container w-full ${isDemo ? 'relative' : ''}`}>
      ...
      <div className="terminal-content w-full h-[300px]">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie data={genreData} cx="50%" cy="50%" labelLine={false} outerRadius={80} innerRadius={40} fill="#8884d8" dataKey="value" label={({
            name,
            percent
          }) => `${name} ${(percent * 100).toFixed(0)}%`}>
              {genreData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}
            </Pie>
            <Tooltip formatter={value => [`${value} games`, 'Count']} contentStyle={{
      ...
      {isDemo && !document.cookie.includes("demo_note_dismissed") && <div className="mt-auto pt-4 text-center">
          <button onClick={() => signInWithSteam()} className="text-sm text-unplayed-mint hover:underline">Connect to Steam to see your Genre breakdown</button>
        </div>}
    </div>;
};
